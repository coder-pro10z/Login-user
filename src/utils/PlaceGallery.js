...
const PlaceGallery = ({place}) => {
    ...
    if(showAllPhotos){
        ...
        <div className='absolute inset-0 font-no  text-black min-h-screen '>
          ...
          <div className=' backdrop-blur-md	 p-8 grid gap-4'>
    ...
    <div>
      ...
      <div className='grid gap-2 grid-cols-[2fr_1fr] rounded-3xl overflow-hidden'>
        ...
        <div className='grid'>{place.photos?.[1] && (<img onClick={()=> setShowAllPhotos(true)} alt='' className='cursor-pointer aspect-square object-cover min-h-[100%]' src={'http://localhost:5000/uploads/'+place.photos[1]}></img>)}
            ...
            <button onClick={()=>setShowAllPhotos(true)} className='flex gap-1 absolute mr-[12%] bottom-2 right-2 py-2 px-2 bg-white rounded-2xl shawdow shadow-md shadow-gray-500 text-black'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
